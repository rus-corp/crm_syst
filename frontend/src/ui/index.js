import CreateNewItemBtn from "./buttons/CreateNewItemBtn";
import SaveBtnComponent from "./buttons/SaveBtnComponent";
import SmallButton from "./buttons/SmallButton";
import ComponentHeader from "./component_header/ComponentHeader";
import Icon from "./icons/Icon";
import { clientIcon, DashBoardIcon, HotelsIcon, ProgramIcon, partnersIcon } from "./icons/icons";
import MenuItem from "./menu_item/MenuItem";
import NotificationComponent from "./notifications/NotificationComponent";
import CreateItemInput from "./profile_inputs/CreateItemInput";
import ProfileInput from "./profile_inputs/ProfileInput";
import TextAreaComponent from "./profile_inputs/TextAreaComponent";
import SwitchComponent from "./switch/SwitchComponent";
import ThreeComponentSwitch from "./switch/ThreeComponentSwitch";

export { ProgramIcon, Icon, MenuItem,
  DashBoardIcon, HotelsIcon,
  ComponentHeader, CreateNewItemBtn,
  SwitchComponent, clientIcon, partnersIcon,
  ThreeComponentSwitch, ProfileInput,
  SmallButton, CreateItemInput, TextAreaComponent,
  SaveBtnComponent, NotificationComponent }
